...
// A dotted, space-free token is an i18n key path, never real copy.
...
function coerceToString(key: string, value: unknown): string {
  if (typeof value === "string") {
    // i18next echoes the key back while its namespace is still loading. Showing
    // raw "home.quick_links.title_plural" text is worse than showing nothing, so
    // render empty until the bundle resolves and the component re-renders.
    if (value === key && looksLikeKeyPath(value)) return "";
    return value;
  }
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    console.warn(
      ...
      }). This is likely a missing key or a namespace-node lookup.`
    );
  }
  return looksLikeKeyPath(key) ? "" : key;
}
