...
export class IssueService extends APIService {
  ...
  /**
   * Sign off on a completed work item. Separate from patchIssue because
   * verification is its own permissioned action, not an editable property —
   * the API rejects verified_at/verified_by on the ordinary update endpoint.
   */
  async verifyIssue(workspaceSlug: string, projectId: string, issueId: string): Promise<TIssue> {
    return this.post(`/api/workspaces/${workspaceSlug}/projects/${projectId}/${this.serviceType}/${issueId}/verify/`)
      .then((response) => response?.data)
      .catch((error) => {
        throw error?.response?.data;
      });
  ...
  /** Withdraw a sign-off that was granted in error. */
  async removeIssueVerification(workspaceSlug: string, projectId: string, issueId: string): Promise<TIssue> {
    return this.delete(`/api/workspaces/${workspaceSlug}/projects/${projectId}/${this.serviceType}/${issueId}/verify/`)
      .then((response) => response?.data)
      .catch((error) => {
        throw error?.response?.data;
      });
