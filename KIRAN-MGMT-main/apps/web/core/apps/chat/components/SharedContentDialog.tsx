...
export function SharedContentDialog({
  ...
  // The original narrowed with `message.attachment!` at each render site; under
  // strictNullChecks the pair is carried alongside the message instead.
  const media = roomMessages.flatMap((message) => {
    const attachment = message.attachment;
    return attachment && isMediaAttachment(attachment) ? [{ message, attachment }] : [];
  });
  const docs = roomMessages.flatMap((message) => {
    const attachment = message.attachment;
    return attachment && !isMediaAttachment(attachment) ? [{ message, attachment }] : [];
  });
