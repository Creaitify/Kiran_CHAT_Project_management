 ...
 *
 * `pageBefore` and `windowIncluding` re-sort the whole in-memory log on every
 * call. That is fine for the client-side merge of an already-loaded window; it
 * is not the paging strategy — the API owns that, and these are what keep
 * optimistic sends in the right place until it answers.
 */

import { decodeCursor, encodeCursor, type Cursor, type Page, type SharedMessage } from "./chat-types";
