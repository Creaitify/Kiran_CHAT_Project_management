...
/**
 * Per-viewer profile photo storage and the client-side image downscaler.
 *
 * `prepareProfilePhoto` earns its place permanently: the group- and profile-photo
 * croppers need a bounded data URL before anything is uploaded, and a 12MP phone
 * photo pasted straight into JSON is not that.
 *
 * `useUserProfilePhoto` is a stopgap. KIRAN owns user avatars server-side, so an
 * avatar cached in localStorage under an id the server never sees will disagree
 * with every other surface in the app. Replace it with the workspace member's
 * real avatar once the chat directory reads from `IUser`. Note too that the
 * `actorUserId !== userId` check in `setOwnPhoto` is a UI affordance, not a
 * permission — it guards a localStorage write on the actor's own machine.
 */
...
const PREFIX = "kiran-chat-profile-photo";
const CHANGE_EVENT = "kiran-chat-profile-photo-change";
...
export function useUserProfilePhoto(userId: string) {
  ...
  // Starts null and fills in from the effect, so the server render and the first
  // client render agree.
