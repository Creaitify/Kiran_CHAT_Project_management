...
/**
 * Rate limiting and token budgets for the AI route.
 *
 * READ THIS BEFORE IMPORTING IT. In the standalone app this ran on the server —
 * its only caller was the h3 handler behind `/api/agent`, and it took a Web
 * `Request` because that is what that handler was handed. Nothing in a browser
 * bundle can be protected by it: the client owns the Maps below, so the client
 * can reset them.
 *
 * KIRAN's server is Django, and DRF already carries the equivalent machinery
 * (`DEFAULT_THROTTLE_CLASSES` / `DEFAULT_THROTTLE_RATES` in
 * `apps/api/plane/settings/common.py`). The AI budget belongs there as a
 * throttle class keyed on the authenticated user, not here. This file is kept
 * only as the reference for the shape that class should have — the token-bucket
 * refill, the retry-after arithmetic and the `estimateTokens` heuristic all
 * transfer. Nothing imports it today, and nothing in the web app should.
 *
 * The original scope note, still accurate for whatever runtime hosts it: this is
 * an in-memory limiter. It is correct for a single instance, does not survive a
 * restart, and does not coordinate across instances. Moving to Redis means
 * replacing the store below, not the call sites.
 */
...
/** Bounds memory growth from unique keys on a long-lived process. */
 ...
 * limiter on an authenticated user id, which on the Django side is free.
 */
