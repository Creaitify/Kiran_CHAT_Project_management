...
// Verification has no store of its own — it is a single server-owned flag that
// the issue refetch picks up — so the service is used directly here rather than
// threading two more actions through the MobX detail store.
...
export type TIssueOperations = {
  ...
  /** Sign off on a completed work item. Admin-only, and never your own completion. */
  verify?: (workspaceSlug: string, projectId: string, issueId: string) => Promise<void>;
  /** Withdraw a sign-off granted in error. */
  removeVerification?: (workspaceSlug: string, projectId: string, issueId: string) => Promise<void>;
...
export const IssueDetailRoot = observer(function IssueDetailRoot(props: TIssueDetailRoot) {
  ...
  const issueOperations: TIssueOperations = useMemo(
    () => ({
      ...
      verify: async (workspaceSlug: string, projectId: string, issueId: string) => {
        try {
          await issueService.verifyIssue(workspaceSlug, projectId, issueId);
          // Refetch rather than patching locally: the server owns verified_at
          // and rejects it on the update endpoint, so the store has no way to
          // derive the timestamp it just generated.
          await fetchIssue(workspaceSlug, projectId, issueId);
          setToast({
            title: t("common.success"),
            type: TOAST_TYPE.SUCCESS,
            message: t("issue.verify.success"),
          });
        } catch (error) {
          console.log("Error in verifying issue:", error);
          setToast({
            title: t("common.error.label"),
            type: TOAST_TYPE.ERROR,
            // The API refuses self-verification and un-completed items with a
            // specific reason; surface it instead of a generic failure.
            message: (error as { error?: string })?.error ?? t("issue.verify.failed"),
          });
        }
      },
      removeVerification: async (workspaceSlug: string, projectId: string, issueId: string) => {
        try {
          await issueService.removeIssueVerification(workspaceSlug, projectId, issueId);
          await fetchIssue(workspaceSlug, projectId, issueId);
          setToast({
            title: t("common.success"),
            type: TOAST_TYPE.SUCCESS,
            message: t("issue.verify.removed"),
          });
        } catch (error) {
          console.log("Error in removing issue verification:", error);
          setToast({
            title: t("common.error.label"),
            type: TOAST_TYPE.ERROR,
            message: (error as { error?: string })?.error ?? t("issue.verify.failed"),
