...
# for self-hosted deployments; set to e.g. "kirancableppl.com" to block specific domains.
...
# Database
#
# DATABASE_URL is the primary, documented path — it is what lets a managed
# Postgres (Neon, RDS, Supabase) be attached by configuration alone. The
# POSTGRES_* block below is the fallback for the local Docker compose stack.
# See CONNECTORS.md at the repository root.
#
# The three knobs are exposed because a *pooled* connection needs different
# values from a direct one, and getting them wrong fails at runtime rather than
# at boot:
#
#   CONN_MAX_AGE                    must stay 0 behind PgBouncer/Neon's pooler;
#                                   persistent connections defeat the pooler and
#                                   exhaust it under load. Raise it only when
#                                   pointing at a direct host.
#   DB_DISABLE_SERVER_SIDE_CURSORS  must be 1 behind a transaction-mode pooler.
#                                   Django's .iterator() opens a server-side
#                                   cursor that the pooler will hand a different
#                                   backend on the next statement, producing
#                                   "cursor does not exist" mid-request.
#   DB_CONN_HEALTH_CHECKS           only meaningful when CONN_MAX_AGE > 0.
if bool(os.environ.get("DATABASE_URL")):
    # Parse database configuration from $DATABASE_URL
    DATABASES = {
        "default": dj_database_url.config(
            conn_max_age=int(os.environ.get("CONN_MAX_AGE", "0")),
            conn_health_checks=os.environ.get("DB_CONN_HEALTH_CHECKS", "0") == "1",
            disable_server_side_cursors=(
                os.environ.get("DB_DISABLE_SERVER_SIDE_CURSORS", "0") == "1"
            ),
        )
    }
else:
