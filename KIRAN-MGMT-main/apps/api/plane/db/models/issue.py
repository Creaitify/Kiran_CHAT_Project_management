...
class Issue(ChangeTrackerMixin, ProjectBaseModel):
    ...
    # Who moved this work item into a completed state. `updated_by` cannot serve
    # this purpose: any later edit overwrites it, so it stops being an answer to
    # "who finished this" the moment anyone touches the item again.
    completed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        related_name="completed_issues",
        null=True,
        blank=True,
    )
    # Sign-off, deliberately kept separate from completion: the worker reports
    # the work done, someone else confirms it. Both halves are cleared together
    # whenever the item leaves a completed state (see _sync_completed_at), so a
    # reopened item can never carry a stale approval.
    ...
    def _sync_completed_at(self, kwargs):
        """Update completion and verification bookkeeping when state changes.

        Completion is recorded automatically so no caller has to remember to set
        it, and so it cannot be forged over the wire (the serializer keeps these
        fields read-only). Verification is only ever *cleared* here — granting it
        is an explicit, separately-permissioned action.
        ...
        touched_fields = {"completed_at", "completed_by", "verified_at", "verified_by"}

        if self.state.group == StateGroup.COMPLETED.value:
            self.completed_at = timezone.now()
            actor = get_current_user()
            # An anonymous or system-driven save (a webhook, a data migration,
            # the seeder) leaves this null rather than attributing the work to
            # whoever happens to be in scope.
            self.completed_by = None if actor is None or actor.is_anonymous else actor
        else:
            # Reopened, cancelled, or moved back to in-progress. Any prior
            # sign-off described work that is no longer finished, so it dies
            # with the completion it was approving.
        ...
        if update_fields is not None:
            kwargs["update_fields"] = list(set(update_fields) | touched_fields)
        return kwargs
