...
class ProjectStatsEndpoint(BaseAPIView):
    ...
    def get(self, request, slug):
        ...
        # NOTE: `completed_issues` counts the "cancelled" group too, so it really
        # means "closed". Existing callers depend on that, so it is left alone and
        # `cancelled_issues` is exposed alongside it -- subtract to get the number
        # of work items that were genuinely finished.
        ...
        if "cancelled_issues" in requested_fields:
            annotations["cancelled_issues"] = (
