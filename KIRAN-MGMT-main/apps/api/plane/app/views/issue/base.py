...
class IssueViewSet(BaseViewSet):
    ...
    # `assignee=True` lets someone update a work item that has been assigned to
    # them even when their project role would not otherwise allow it — a Guest
    # can report progress on their own work without gaining write access to the
    # rest of the project.
    @allow_permission(allowed_roles=[ROLE.ADMIN, ROLE.MEMBER], creator=True, assignee=True, model=Issue)
...
class IssueVerificationEndpoint(BaseAPIView):
    """Sign-off on a completed work item.

    Deliberately a separate endpoint rather than a writable field: verification
    is a different act from completion, needs a different permission, and must
    not be settable by whoever is editing the item's other properties.

    Only project admins may verify, and nobody may verify their own completion —
    the point of the second step is that a second pair of eyes saw the work.
    """
    ...
    def post(self, request, slug, project_id, issue_id):
        ...
        if issue.completed_at is None:
            return Response(
                {"error": "Only a completed work item can be verified. Move it to a completed state first."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if issue.completed_by_id == request.user.id:
            return Response(
                {"error": "A work item must be verified by someone other than the person who completed it."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if issue.verified_at is not None:
            return Response(
                {"error": "This work item has already been verified."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        # Bypass the state-change machinery in Issue.save(); nothing about the
        # item's state is changing, only its sign-off.
        issue.save(update_fields=["verified_at", "verified_by", "updated_at", "updated_by"])

        issue_activity.delay(
            type="issue.activity.updated",
            requested_data=json.dumps({"verified_at": str(issue.verified_at)}, cls=DjangoJSONEncoder),
            actor_id=str(request.user.id),
    ...
    def delete(self, request, slug, project_id, issue_id):
        """Withdraw a sign-off, e.g. it was granted in error."""
        issue = Issue.objects.filter(workspace__slug=slug, project_id=project_id, pk=issue_id).first()

        if not issue:
            return Response({"error": "Work item not found"}, status=status.HTTP_404_NOT_FOUND)

        if issue.verified_at is None:
            return Response(
                {"error": "This work item has not been verified."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        issue_activity.delay(
            type="issue.activity.updated",
            requested_data=json.dumps({"verified_at": None}, cls=DjangoJSONEncoder),
            actor_id=str(request.user.id),
            issue_id=str(issue_id),
            project_id=str(project_id),
            current_instance=current_instance,
