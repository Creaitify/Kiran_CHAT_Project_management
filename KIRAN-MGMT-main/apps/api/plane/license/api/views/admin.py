...
from plane.authentication.utils.demo_mode import is_demo_mode, provision_demo_user
...
class InstanceAdminSignInEndpoint(View):
    ...
    def post(self, request):
        ...
        # KCMS demo mode: provision the admin on first sign in so any email works.
        if is_demo_mode():
            if not user:
        ...
        # Check password of the user. Demo mode deliberately does NOT bypass this:
        # the god-mode console can reconfigure the whole instance, so it always
        # requires the real password even on a local demo instance.
